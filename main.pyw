...
# Draw phpvms-acars UI
class App:

    def __init__(self, root):
        self.root = root
        self.root.iconbitmap('images/Favicon.ico')
        self.root.title('phpvms-acars - ' + config.version)
    ...
    def menu(self):
        ...
        self.helpMenu.add_command(label='About phpvms-acars', command=self.about)
    ...
    def login(self):
        ...
        self.headerLbl.config(text="phpvms-acars - Login")
        ...
        self.registerLink = tk.Label(self.loginFrame, text='New to phpvms-acars? Add an account', fg="#676767")
    ...
    def register(self):
        ...
        self.headerLbl.grid_forget()
        self.headerLbl.config(text="phpvms-acars - Register")
        ...
        tk.Label(self.registerFrame, text="Airline Website (Format: https://example.com - no slash at the end)").grid(row=6, column=1, sticky="w", pady=(0, 5))
    ...
    def accounts(self):
        ...
        self.accountsMenu.title('phpvms-acars - Manage Accounts')
    ...
    def dashboard(self):
        ...
        self.Log('View phpvms-acars output here')

        self.dbBody.grid(row=self.dashboardViewRow, column=self.dashboardViewCol, sticky=self.dashboardViewSticky, pady=(5, 0))
        tk.Label(self.dbBody, text="Welcome to phpvms-acars", font=self.h1, fg="#676767").grid(row=0, column=0)
        ...
        tk.Label(self.dbBody, text="3. Enter your sim, enabled phpvms-acars, and click \"Start\". Update your enroute time as needed", font=self.h4).grid(row=8, column=0, sticky="w")
    ...
    def about(self):
        about = tk.Toplevel()
        about.title('About phpvms-acars')
        about.iconbitmap('images/Favicon.ico')

        self.aboutFrame = tk.Frame(about)
        self.aboutFrame.grid(row=0, column=0)

        self.header = tk.Label(self.aboutFrame, text="phpvms-acars " + config.version, font="Arial")
        self.header.grid(row=0, column=0)
        self.bio = tk.Label(self.aboutFrame, text="phpvms-acars is based of xACARS v1.0.0-beta1. xACARS was originally developed by Speed_Limit75.\nThis version features improvements by Henry Shires, you can check out his VA at: https://estafetava.com")
        ...
        self.text = tk.Label(self.aboutFrame, text="Found issues? Let us know! Just go to https://github.com/ajh123/phpvms-acars/issues")
    ...
    def settings(self):
        ...
        # Draw window
        self.settingsWin.title('phpvms-acars Settings')
    ...
    def setupFlight(self):
        ...
        if data == '{"data":[]}':
            tk.Label(self.bidsFrame, text="No bids found! Please place a bid on your airline's website and restart phpvms-acars.").grid(row=1, column=0)
            self.data = "No bids"
        else:
    ...
    def selectBid(self):
        ...
        if not(selBid.__eq__("Select a Bid")):
            ...
            tk.messagebox.showerror('phpvms-acars Error', 'Please select a bid!')
    ...
    def doDelete(self):
        proceed = tk.messagebox.askyesno('phpvms-acars', 'Are you sure you want to delete ' + self.delAirline.get() + ' from phpvms-acars?')
        if proceed:
            if not(self.isInFlight()):
                ...
                tk.messagebox.showerror('phpvms-acars Error', 'Unable to delete account. Complete your flight first!')
    ...
    # Check if the current version of phpvms-acars is the latest, published version
    def doCheckForUpdates(self):
        if (not(web.isLatestVersion())):
            tk.messagebox.showinfo("phpvms-acars Update", "Update available! You can download the latest version of phpvms-acars from https://github.com/3dash/phpvms-acars/releases")
        else:
            tk.messagebox.showinfo("phpvms-acars Update", "No updates available")
    ...
    def connectionTest(self):
        ...
        if isSuccess:
            tk.messagebox.showinfo("phpvms-acars", "Connected to Sim!")
            return True
        else:
            tk.messagebox.showerror('phpvms-acars Error','Unable to connect to sim. ' + isSuccess)
            return False
        track.endTrack()
    ...
    def isInFlight(self):
        try:
            ...
            tk.messagebox.showerror('phpvms-acars Error', e)
            return False
    ...
    def restoreToDefaults(self):
        ...
        self.restoreToDefault = tk.messagebox.askyesno("phpvms-acars","Are you sure you want to restore to defaults?")
        self.settingsWin.lift()

        if self.restoreToDefault == True:
            os.remove("settings.ini")
            config.reloadIni()
            tk.messagebox.showinfo("phpvms-acars","Restored defaults.")
